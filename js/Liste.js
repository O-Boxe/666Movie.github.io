async function showList() {
    ...
    for (let i = 0; i < film.length; i++) {
        ...
        var genre = genres[data.genres[0]];
        var genre2 = genres[data.genres[1]];
        var genre3 = genres[data.genres[2]];
        ...
        if (localStorage.getItem("enregistrement").split(';').includes(`${data.id}`)) {
            var saved = 'saved'
       }else{
            var saved = ''
       }
        ...
        flyers =   `<div class="flyer" style="background:linear-gradient(to top,rgb(0, 0, 0) ,rgba(0, 0, 0, 0.123)),url(${imageLink});">
                        <svg id="saveBtn${data.id}"  class="save ${saved}" onclick="save(${data.id}, true)" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
